...
def show_table(df_original, clear_selection=False):
    df = df_original.copy()
    ...
    columns.remove('tarefa_status')
    columns.insert(columns.index('tarefa'), 'tarefa_status')
    df = df[columns]
    ...
    response = AgGrid(
        ...
        key="tabela_resetada" if clear_selection else "tabela_principal",
        height=350,
